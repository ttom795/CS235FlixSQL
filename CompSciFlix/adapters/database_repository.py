...
from CompSciFlix.domain.model import User, Article, Comment, Tag
...
class SqlAlchemyRepository(AbstractRepository):
    ...
    def get_articles_by_actor(self, target_actor: str) -> List[Article]:
        articles = self._session_cm.session.query(Article).filter(Article._date == target_actor).all()
        return articles

    def get_articles_by_director(self, target_director: str) -> List[Article]:
        articles = self._session_cm.session.query(Article).filter(Article._director == target_director).all()
        return articles

    def get_articles_by_title(self, target_title: str) -> List[Article]:
        articles = self._session_cm.session.query(Article).filter(Article._title == target_title).all()
        return articles
    ...
    def get_article_by_id(self, id):
        articles = self._session_cm.session.query(Article).filter(Article._id == id)
        print(articles)
        return articles
...
def article_record_generator(filename: str):
    with open(filename, mode='r', encoding='utf-8-sig') as infile:
        ...
        for row in reader:
            ...
            article_tags = article_data[2].split(",")
            ...
            ad = article_data
            x = [ad[0], int(ad[6]), ad[1], ad[3], "", ad[4], ad[7], ad[8], str(ad[5].split(","))]
            yield x
...
def populate(engine: Engine, data_path: str):
    ...
    insert_articles = """
        ...
        id, date, title, first_para, image_link, director, runtime, rating, actors)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"""
    cursor.executemany(insert_articles, article_record_generator(os.path.join(data_path, 'Data1000Movies.csv')))
    ...
    insert_comments = """
        ...
        id, user_id, article_id, comment, timestamp, rating)
        VALUES (?, ?, ?, ?, ?, ?)"""
