...
from CompSciFlix.adapters.repository import AbstractRepository, RepositoryException
from CompSciFlix.domain.model import Article, Tag, User, Comment, make_tag_association, make_comment
...
class MemoryRepository(AbstractRepository):
    ...
    def get_articles_by_director(self, target_director: str) -> List[Article]:
        matching_articles = list()
        for article in self._articles:
            name = article.director.director_full_name.lower()
            if target_director.lower() in name or target_director.lower() == name:
                matching_articles.append(article)
        return matching_articles
    ...
    def get_articles_by_actor(self, target_actor: str) -> List[Article]:
        matching_articles = list()
        for article in self._articles:
            actorlist = [i.actor.lower() for i in article.actors]
            for item in actorlist:
                if target_actor.lower() in item:
                    matching_articles.append(article)
        return matching_articles
